...
def handle_client(client):  # target of client thread
    ...
    clients[client] = name
    # notify new client about already connected users
    if clients.__len__() > 1:
        broadcast(bytes("Here are the current occupants: ", "utf8"))
        for con_client in clients:
            broadcast(bytes('\n'.join(clients[con_client]), "utf8"))
